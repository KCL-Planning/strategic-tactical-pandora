...
#include "knowledge_msgs/ActionEnergyEstimator.h"
...
namespace PandoraKCL {
	 ...
	 */
	void runEnergyClient()
	{
		ros::NodeHandle nh("~");
		ROS_INFO("KCL: --- energyTester ---");
		...
		// requesting from "/knowledge/energy/action"
		ros::ServiceClient energyClient = nh.serviceClient<knowledge_msgs::ActionEnergyEstimator>("/knowledge/energy/action");
		energyClient.waitForExistence();
		...
		while (ros::ok() && PandoraKCL::actionList.size() > PandoraKCL::currentAction) {
			...
			// get remaining energy estimate
			knowledge_msgs::ActionEnergyEstimator srv;
			std::cout << srv.request.actions.size() << std::endl;
			for(size_t i = currentAction; i<PandoraKCL::actionList.size(); i++)
				srv.request.actions.push_back(PandoraKCL::actionList[i]);
			if (!energyClient.call(srv))
				ROS_ERROR("KCL: Failed to call service /knowledge/energy/action");
			else
				ROS_INFO("KCL: Estimated energy for %i actions is: %f Joules",
						(int)(PandoraKCL::actionList.size() - PandoraKCL::currentAction),
						srv.response.energy);
		...
		ROS_INFO("KCL: Energy Estimation Complete");
	}
