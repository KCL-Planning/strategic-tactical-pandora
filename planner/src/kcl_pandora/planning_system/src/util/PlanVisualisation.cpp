...
namespace PandoraKCL {
	...
	void initVis() {
		...
		char appname[] = "SQUIRREL TIDY-NATOR 500: Prince Push";
		...
		window = glutCreateWindow("SQUIRREL TIDY-NATOR 500: Prince Push");  
	...
	void drawAction(planning_msgs::ActionDispatch &msg, float startTime, size_t level) {

		bool currentPlan = (level == PandoraKCL::planningAttempts-1);

		float r, g, b;
		if((size_t)msg.action_id < PandoraKCL::currentAction) {
			r = (25.0/255.0f); g = (25.0/255.0f); b = (112.0/255.0f);
		}
		else if(PandoraKCL::currentAction == (size_t)msg.action_id) {
		...
		if(PandoraKCL::currentAction == (size_t)msg.action_id && currentPlan) {

			int text_baseline = 100 + (barHeight+14)*PandoraKCL::planningAttempts;
			std::stringstream ss;
	...
	void draw() {
		...
		if(PandoraKCL::totalPlanDuration > largestDuration)
			largestDuration = PandoraKCL::totalPlanDuration;
		...
		float time = 0.0;
		for(size_t i=0;i<PandoraKCL::planList.size();i++) {
			time = 0.0;
			for(size_t j=0; j<PandoraKCL::planList[i].size(); j++) {
				drawAction(PandoraKCL::planList[i][j], time, i);
				time = time + PandoraKCL::planList[i][j].duration;
			}
		}

		time = 0.0;
		for(size_t i=0; i<PandoraKCL::actionList.size(); i++) {
			drawAction(PandoraKCL::actionList[i], time, PandoraKCL::planningAttempts-1);
			time = time + PandoraKCL::actionList[i].duration;
		}

		if(PandoraKCL::dispatchPaused)
