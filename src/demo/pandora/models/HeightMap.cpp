...
bool HeightMap::getCollisions(DreadedPE::Entity& entity, const glm::vec3& begin, const glm::vec3& end, std::vector<DreadedPE::CollisionInfo>& info) const
{
	bool found_collision = false;
	if (&entity != this)
	{
...
bool HeightMap::doesCollide(DreadedPE::Entity& entity, DreadedPE::CollisionInfo& info) const
{
	if (&entity != this)
	{
		for (std::vector<const DreadedPE::Plane*>::const_iterator ci = entity.getCollisionChecker().getPlanes().begin(); ci != entity.getCollisionChecker().getPlanes().end(); ++ci)
		{
			const DreadedPE::Plane* plane = *ci;
...
bool HeightMap::doesCollide(DreadedPE::Entity& entity, const glm::vec3& begin, const glm::vec3& end, DreadedPE::CollisionInfo& info) const
{
	if (this != &entity)
	{
...
bool HeightMap::doesCollide(DreadedPE::Entity* entity, const glm::vec3& begin, const glm::vec3& end, float effective_width) const
{
	if (entity != this)
	{
		glm::mat4 m = glm::inverse(getCompleteTransformation());
...
bool HeightMap::doesCollide(DreadedPE::Entity* entity, const glm::vec3& point) const
{
	if (entity != this)
	{
		glm::mat4 m = glm::inverse(getCompleteTransformation());
