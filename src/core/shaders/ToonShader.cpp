...
ToonShader* ToonShader::shader_ = NULL;

ToonShader::ToonShader(const std::string& vertex_shader, const std::string& fragment_shader)
	: LightShader(vertex_shader, fragment_shader)
{

}

void ToonShader::prepareToRender(const SceneLeafModel& model_node, const glm::mat4& view_matrix, const glm::mat4& model_matrix, const glm::mat4& projection_matrix, const std::vector<const SceneLeafLight*>& lights)
{
...
ToonShader& ToonShader::getShader()
{
	if (shader_ == NULL)
	{
		shader_ = new ToonShader("shaders/toon.vert", "shaders/toon.frag");
		
		// Load the shader.
		if (!shader_->initialize())
		{
			std::cerr << "Failed to initialise the toon shader." << std::endl;
			exit(1);
		}
