...
BasicShadowShader* BasicShadowShader::shader_ = NULL;

BasicShadowShader::BasicShadowShader(const std::string& vertex_shader, const std::string& fragment_shader)
	: LightShader(vertex_shader, fragment_shader)
{

}

void BasicShadowShader::prepareToRender(const SceneLeafModel& model_node, const glm::mat4& view_matrix, const glm::mat4& model_matrix, const glm::mat4& projection_matrix, const std::vector<const SceneLeafLight*>& lights)
{
...
BasicShadowShader& BasicShadowShader::getShader()
{
	if (shader_ == NULL)
	{
		shader_ = new BasicShadowShader("shaders/animated-shadow.vert", "shaders/basic-shadow.frag");
